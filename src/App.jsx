...
/* ============================ STYLES (single-file: injected, no separate .css) ============================ */
const CSS = `
...
:root{
    ...
    /* status accent — overwritten on the root by React */
    --accent:#34F5C5;
    --accent-2:#16d6ac;
  ...
  *{box-sizing:border-box;}
  ...
  @media (max-width:900px){
    .col-hero,.col-chart,.col-7,.col-5{grid-column:span 12;}
...
const STORE_KEY = "credit-runway-v1";
const C2D = (c) => c / (BUDGET / DOLLARS); // credits -> dollars

const STATUS = {
  green: {
    key: "green", label: "ON PACE", accent: "#34F5C5", a2: "#16d6ac", glow: "rgba(52,245,197,.55)", soft: "rgba(52,245,197,.14)", ink: "#062b24",
    msg: "Safe to continue utilizing AI services."
  },
  yellow: {
    key: "yellow", label: "WARNING", accent: "#FFB838", a2: "#f59e0b", glow: "rgba(255,184,56,.5)", soft: "rgba(255,184,56,.13)", ink: "#2b1d02",
    msg: "Approaching daily cap. Consider pacing your usage."
  },
  red: {
    key: "red", label: "ALERT", accent: "#FF4D6D", a2: "#e11d48", glow: "rgba(255,77,109,.5)", soft: "rgba(255,77,109,.13)", ink: "#2b0610",
...
function getCycle(now = new Date()) {
  ...
  const remainingDays = daysInMonth - dom + 1;     // today counts
  const monthKey = `${y}-${String(m + 1).padStart(2, "0")}`;
  const monthName = now.toLocaleString("en-US", { month: "long" });
  const todayKey = `${monthKey}-${String(dom).padStart(2, "0")}`;
  return { y, m, daysInMonth, dom, remainingDays, monthKey, monthName, todayKey };
}
...
function labelFor(key) {
  ...
  return {
    short: dt.toLocaleString("en-US", { month: "short", day: "numeric" }),
    dow: dt.toLocaleString("en-US", { weekday: "short" }),
  };
}
const fmt = (n) => Math.round(n).toLocaleString("en-US");
const fmtMoney = (c) => "$" + C2D(c).toLocaleString("en-US", { maximumFractionDigits: 0 });
...
function writeStore(obj) {
  try { localStorage.setItem(STORE_KEY, JSON.stringify(obj)); } catch (e) { /* sandbox / private mode: in-memory only */ }
}
function seedMock(cycle) {
  const ideal = BUDGET / cycle.daysInMonth;
  const factors = [0.92, 1.18, 0.78]; // -3, -2, -1 days
  const out = [];
  for (let i = 0; i < 3; i++) {
    const d = cycle.dom - (3 - i);
    if (d < 1) continue;                 // clamp near month start
    out.push({ id: "m" + i, date: fmtKey(cycle.y, cycle.m, d), amount: Math.round(ideal * factors[i]) });
  }
  return out;
}
...
function Gauge({ monthlyPct, idealPct, status }) {
  const W = 320, cx = 160, cy = 158, R = 126, SW = 20;
  const L = Math.PI * R;
  const animPct = useEased(Math.min(100, monthlyPct), 900, 0);
  ...
  return (
    <svg viewBox={`0 0 ${W} 178`} className="chart-svg" style={{ maxWidth: "340px" }} role="img"
      aria-label={`${Math.round(monthlyPct)} percent of monthly budget consumed`}>
      <defs>
...
/* ============================ BURN-DOWN CHART ============================ */
function BurnChart({ cumulative, today, daysInMonth, projTotal }) {
  ...
  const yForRaw = (c) => yTop + (1 - c / BUDGET) * plotH;
  ...
  const yTicks = [0, 0.25, 0.5, 0.75, 1].map(f => ({ c: f * BUDGET, label: f === 0 ? "0" : fmt(f * BUDGET / 1000) + "k" }));
  const xTicks = [1, Math.ceil(daysInMonth / 2), daysInMonth];

  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="chart-svg" role="img" aria-label="Cumulative burn-down versus ideal pace">
      <defs>
      ...
      {yTicks.map((t, i) => (
        <g key={i}>
          <line x1={padL} y1={yFor(t.c)} x2={W - padR} y2={yFor(t.c)} stroke="var(--line)" strokeWidth="1" opacity={i === 4 ? .8 : .4} />
          <text x={padL} y={yFor(t.c) - 4} fill="var(--dim)" fontSize="9.5" fontFamily="JetBrains Mono">{t.label}</text>
        </g>
      ))}
      {/* x ticks */}
      {xTicks.map((d, i) => (
      ...
      {/* ideal glidepath */}
      <line x1={xFor(0)} y1={yFor(0)} x2={xFor(daysInMonth)} y2={yFor(BUDGET)}
        stroke="var(--steel)" strokeWidth="2" strokeDasharray="6 5" opacity=".75" />
      {/* today marker */}
      <line x1={todayX} y1={yTop} x2={todayX} y2={yBot} stroke="var(--line-bright)" strokeWidth="1" strokeDasharray="2 4" />
      {/* area + actual */}
      <path d={area} fill="url(#areaFill)" />
      <path d={actual} fill="none" stroke="var(--accent)" strokeWidth="2.6" strokeLinejoin="round" strokeLinecap="round" />
      {/* projection */}
      {today < daysInMonth &&
...
function App() {
  ...
  const [entries, setEntries] = useState(() => {
    const s = readStore();
    if (!s) return seedMock(cycle);                       // first ever load -> mock
    if (s.month !== cycle.monthKey) return [];            // new cycle -> reset
    return s.entries || [];
  });
  ...
  const dailyTarget = cycle.remainingDays > 0 ? remaining / cycle.remainingDays : 0;
  ...
  const status = useMemo(() => {
    ...
    if (dailyTarget <= 0) return STATUS.red;
  ...
  // apply status accent to the whole deck (the signature)
  useEffect(() => {
  ...
  const idealPct = (cycle.dom / cycle.daysInMonth) * 100;     // where we *should* be today
  const avgPerDay = totalUsed / cycle.dom;
  const projTotal = Math.round(avgPerDay * cycle.daysInMonth); // end-of-month at current velocity
  const projDelta = projTotal - BUDGET;

  // cumulative series for chart
  ...
  // history rows (desc), with vs-average pace tag
  const idealDaily = BUDGET / cycle.daysInMonth;
  const rows = useMemo(() => {
    return [...entries].sort((a, b) => b.date.localeCompare(a.date)).map(e => {
      const lab = labelFor(e.date);
      return {
        ...e, ...lab, isToday: e.date === cycle.todayKey,
        pct: (e.amount / BUDGET) * 100, over: e.amount > idealDaily * 1.1
      };
    });
  ...
  const addEntry = useCallback(() => {
    ...
    setEntries(prev => {
      ...
      if (i >= 0) { const next = [...prev]; next[i] = { ...next[i], amount: next[i].amount + p.credits }; return next; }
  ...
  const saveEdit = (id) => {
    const v = parseInt(String(editVal).replace(/,/g, ""), 10);
  ...
  /* -------- input live hint -------- */
  let hint = <span>Type credits like <b>1500</b>, or a share of budget like <b>20%</b>.</span>, hintCls = "";
  if (raw.trim()) {
    if (parsed.ok) {
      hintCls = "ok";
    ...
    <>
      ...
      <div className="wrap" data-status={status.key}>
        ...
        <header className="deck-head">
          ...
          <div className="head-right">
            <div className="cycle-chip">
              ...
              {cycle.monthName} · resets in <b>{cycle.remainingDays}d</b>
            </div>
        ...
        <div className="grid">
          ...
          <section className="card lift col-hero gauge-wrap">
            ...
            <div className="gauge-stage">
              <Gauge monthlyPct={monthlyPct} idealPct={idealPct} status={status} />
            ...
            <div className="gauge-legend">
              ...
              <span><i className="lg-ideal"></i> Ideal today ({Math.round(idealPct)}%)</span>
            </div>
          </section>

          {/* BURN-DOWN CHART */}
          <section className="card col-chart chart-card">
            <div className="card-label">Cumulative Burn-Down · Glidepath</div>
            <div className="chart-legend" style={{ marginTop: "12px" }}>
              <span><i className="ck actual"></i> Actual</span>
              <span><i className="ck ideal"></i> Ideal pace</span>
              <span><i className="ck proj"></i> Projected ({fmt(projTotal)})</span>
            </div>
            <BurnChart cumulative={cumulative} today={cycle.dom} daysInMonth={cycle.daysInMonth} projTotal={projTotal} />
          </section>
          ...
          <Stat label="Days Left in Cycle"
            value={cycle.remainingDays}
            small={`of ${cycle.daysInMonth}`}
            note={`≈ ${fmt(dailyTarget)} credits / day target`} />
          ...
          <div className="banner" data-status={status.key}>
            ...
            <div className="banner-txt">
              ...
              <span>{depleted ? "No credits remain this cycle. Usage now risks overshooting your $400 cap before reset." : status.msg}</span>
            </div>
          </div>
            ...
            <div className="input-row">
              <div className="field">
                ...
                <input value={raw} onChange={e => setRaw(e.target.value)} onKeyDown={onKey}
                  placeholder="1500   or   20%" inputMode="text" aria-label="Credits or percentage used" />
              </div>
              <button className="btn" onClick={addEntry} disabled={!canAdd}>Add Entry</button>
            </div>
            <div className={"field-hint " + hintCls}>{hint}</div>
            <div className="quick">
          ...
          <section className="card col-12">
            <div className="hist-head" onClick={() => setHistOpen(o => !o)}>
              <div className="card-label" style={{ marginBottom: 0 }}>Daily Log · {cycle.monthName}</div>
                ...
                <table className="table">
                  <thead>
                    <tr>
                      ...
                      <th className="hide-sm">Vs avg pace</th>
                  ...
                  <tbody>
                    {rows.map(r => (
                      <tr key={r.id}>
                        ...
                        <td className="r">
                          <span className={"pace-tag " + (r.over ? "over" : "under")}>{r.over ? "over" : "on"}</span>
                        </td>
                        <td className="r">
                          <div style={{ display: "flex", gap: "6px", justifyContent: "flex-end" }}>
                            {editId === r.id ? (
                              <button className="icon-btn" onClick={() => saveEdit(r.id)} title="Save">{Ic.check}</button>
                            ) : (
                              <button className="icon-btn" onClick={() => startEdit(r)} title="Edit">{Ic.edit}</button>
                            )}
                            <button className="icon-btn danger" onClick={() => delEntry(r.id)} title="Delete">{Ic.trash}</button>
                          </div>
                        </td>
                      </tr>
                    ))}
        ...
        <footer className="deck-foot">
          Auto-resets on the 1st · data stored locally in your browser · <b>100 credits = $1</b>
        </footer>
      </div>
    </>
  );
